...
class RegisterAfterDocumentOtpController extends GetxController {
  ...
  Future<void> sendOTP() async {
    RegisterAfterDocumentController registerController = Get.find();
    String resPhone = registerController.phoneNumberController.text;
    ...
    Response response =
    await NotifyService().sendOtpMessage(message: otp + mes, phoneNumber: resPhone, configId: configId);
  ...
  Future<void> submit() async {
    ...
    if (state != null && state.validate()) {
      ...
      timer.cancel();
      RegisterAfterDocumentController registerController = Get.find();
      registerController.submit(ignoreOtp: true, callbackError: () => Get.back());
      disableResent.value = true;
    } else {
      disableResent.value = false;
    }
